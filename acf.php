...
add_action('init', function () {
	add_filter(
		...
		function ($pre, $url, $request_args) {
			if (
				is_string($url) &&
				strpos($url, 'https://connect.advancedcustomfields.com/') !== false
			) {
...
if (!class_exists('ACF')) {
	...
	class ACF
	{
		...
		public function initialize()
		{
			...
			if (
				$first_activated_version &&
				version_compare($first_activated_version, '6.3', '>=')
			) {
		...
		public function deactivate_other_instances($plugin)
		{
			...
			if (is_multisite() && is_network_admin()) {
				$active_plugins = (array) get_site_option(
